...
if not st.session_state.logged_in:
    ...
    with center_col:
        ...
        with t_reg:
            ...
            npw = st.text_input("Yeni Şifre", type="password")
            if st.button("KAYIT TALEBİ GÖNDER", use_container_width=True):
                users = pd.read_csv(USER_DB)
                if nu in users['Username'].values: st.warning("Bu kullanıcı zaten mevcut.")
                else:
    ...
    with st.sidebar:
        ...
        st.markdown(f"""
            <div class="user-profile">
                <img src="{img_display}" style="width:80px; height:80px; border-radius:50%; object-fit:cover; border:3px solid #00D1FF; margin-bottom:10px;">
                <br><small style="color:#64748B;">{role_tag}</small>
                <div style="font-size:18px; font-weight:800; color:#1E293B;">{st.session_state.u_data['Name'].upper()}</div>
                <div style="color:#00D1FF; font-size:11px; font-weight:700;">PREMIUM PLUS</div>
            </div>
        """, unsafe_allow_html=True)
    ...
    if "DASHBOARD" in menu:
        ...
        if not my_port.empty:
            ...
            st.subheader("📈 Piyasa Trendi")
            sel_asset = st.selectbox("Varlık Seç", my_port['Kod'].unique())
            sym_s = f"{sel_asset}.IS" if my_port[my_port['Kod']==sel_asset]['Kat'].iloc[0] == "Hisse" else f"{sel_asset}-USD"
            ...
            fig.update_layout(template="plotly_white", height=450, margin=dict(l=0,r=0,b=0,t=0))
    ...
    # --- 7. ANALİZLER ---
        ...
        if not my_port.empty:
            my_port['Toplam Değer'] = my_port['Adet'] * my_port['Maliyet']
            col1, col2 = st.columns(2)
            with col1:
                fig_pie = px.pie(my_port, values='Toplam Değer', names='Kod', hole=0.4, title="Varlık Dağılımı")
                st.plotly_chart(fig_pie, use_container_width=True)
            with col2:
                fig_bar = px.bar(my_port, x='Kod', y='Toplam Değer', color='Kat', title="Kategori Bazlı Büyüklük")
                st.plotly_chart(fig_bar, use_container_width=True)
            
        else: st.warning("Analiz için veri bulunamadı.")

    # --- 8. TAKVİM ---
    elif "TAKVİM" in menu:
        st.title("📅 Ekonomik Takvim")
        evs = [{"Tarih": "15 Ocak", "Olay": "ABD Enflasyon Verisi", "Etki": "Kritik 🔥"}, 
               {"Tarih": "22 Ocak", "Olay": "TCMB Faiz Kararı", "Etki": "Yüksek 🚀"}]
        st.table(evs)

    # --- 9. HABERLER ---
    elif "HABERLER" in menu:
        st.title("📰 Canlı Finans Haberleri")
        feed = feedparser.parse("https://www.haberturk.com/rss/kategori/ekonomi.xml")
        for entry in feed.entries[:8]:
            with st.expander(f"📌 {entry.title}"):
                st.write(entry.published)
                st.markdown(f"[Haberin Tamamı]({entry.link})")

    # --- 10. AYARLAR (FOTOĞRAF YÜKLEME DAHİL) ---
    elif "AYARLAR" in menu:
        st.title("⚙️ Terminal Ayarları")
        with st.expander("🖼️ Profil Fotoğrafını Değiştir", expanded=True):
            up_file = st.file_uploader("Bir resim seçin", type=["png", "jpg", "jpeg"])
            if up_file:
                with open(f"avatars/{st.session_state.u_data['Username']}.png", "wb") as f:
                    f.write(up_file.getbuffer())
                st.success("Fotoğraf yüklendi!")
                if st.button("Uygula"): st.rerun()
        ...
        if not pend.empty:
            for i, r in pend.iterrows():
                ...
                if c2.button("ONAYLA", key=r['Username']):
                    ...
                    st.rerun()
        else: st.info("Onay bekleyen kullanıcı yok.")
